...
for var, info in vars.items():
    ...
    if speedy.ndim == 3:
        for i in range(speedy.shape[2]):
            ...
            fig.suptitle(f'{info[0]} [{info[1]}] - field difference  - model level {i+1}')

            plt.savefig(
                os.path.join(output_path, f'{var}_annual_field_diff_level_{i+1}.png')
            )
            plt.close()
    else:
        fig, (ax1, ax2) = plt.subplots(
        ...
        plot_map(ax1, diff.T, 'Difference (Hybrid - SPEEDY)')
        plot_t_test(ax2, t_test_statistics.T, 'Welch\'s t-test (pixel-wise)')
        fig.suptitle(f'{info[0]} [{info[1]}] - field difference')
