...
hybrid = xr.load_dataset(os.path.join(hybrid_path, f'{runs[0]}_precip.nc'))['precip']
speedy = xr.load_dataset(os.path.join(speedy_path, f'{runs[1]}_precip.nc'))['precip']
...
gpcp = xr.open_dataset(os.path.join(GPCP_path, f"{file_name}.nc"))['precip']
...
# Hyrbid Precip
ax2 = fig.add_subplot(gs[0, 3:6], projection=ccrs.PlateCarree(central_longitude=180))
plot_map(ax2, hybrid, 
            f'Hybrid', 
            'mm/day', 0, 15, 1, 25)
...
ax3 = fig.add_subplot(gs[2, 0:3], projection=ccrs.PlateCarree(central_longitude=180))
plot_map(ax3, abs(hybrid_diff) - abs(speedy_diff), 
            '|Hybrid - GPCP| - |SPEEDY - GPCP|', 
            'mm/day', np.min(hybrid_diff - speedy_diff), np.max(hybrid_diff - speedy_diff), 2, 40)
# Differences of Stds
...
plt.savefig(os.path.join(hybrid_path, "Precip_std.png"), dpi = 300, bbox_inches='tight')
plt.show()
