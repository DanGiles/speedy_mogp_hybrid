...
for season in seasons:
    for var, info in vars.items():
        ...
        if speedy.ndim == 3:
            ...
            #     nature_weighted = nature.T*np.cos(np.radians(lat).reshape(-1,1))
            #     fusion_weighted = fusion.T*np.cos(np.radians(lat).reshape(-1,1))

            #     nature_weighted_mean = np.mean(nature_weighted)
            #     fusion_weighted_mean = np.mean(fusion_weighted)

            #     with open(f'{output_path}/{var}_{season}.txt', 'w') as file:
            #         file.write(f'Variable - {var} - {info[0]} ({info[1]})\n')
            #         file.write(f'Fusion: {fusion_weighted_mean}\n')
            #         file.write(f'Nature: {nature_weighted_mean}\n')
            #         file.write(f'Difference: {fusion_weighted_mean - nature_weighted_mean} (Fusion-Nature)')
            ...
            #     if info[1] == 'fraction':
            #         kwargs_colorbar['ticks'] = [0, 0.2, 0.4, 0.6, 0.8, 1]
            #         # kwargs_contourf['levels'] = [0, 0.2, 0.4, 0.6, 0.8, 1]
            #         # kwargs_contourf['levels'] = 10
            #         kwargs_contourf['levels'] = np.linspace(0, 1, 10, endpoint=True)
            #         kwargs_contourf['vmin'] = 0
            #         kwargs_contourf['vmax'] = 1
            #     heatmap = plot_map(axes[0], nature.T*np.cos(np.radians(lat).reshape(-1,1)), 'Nature', **kwargs_contourf)
            #     heatmap = plot_map(axes[1], fusion.T*np.cos(np.radians(lat).reshape(-1,1)), 'Fusion', heatmap, **kwargs_contourf)

            #     fig.colorbar(heatmap, ax=axes, **kwargs_colorbar)
            #     fig.suptitle(f'{info[0]} [{info[1]}] - latitude weighted field mean in {season}')

            #     plt.savefig(
            #         os.path.join(output_path, f'{var}_{season}_weighted_field_means.png')
            #     )
            #     plt.close()
